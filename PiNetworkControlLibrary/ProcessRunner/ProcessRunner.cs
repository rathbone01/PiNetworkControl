    ...
    public class ProcessRunner : IDisposable
    {
        ...
        public async Task ExecuteAsync(string argsString, Stream? target)
        {
            ...
            try
            {
                ...
                {
                    await _process.StandardError.BaseStream.CopyToAsync(target, _processSettings.BufferSize, _cancellationTokenSource.Token);
                }
                else
                {
                    await _process.StandardOutput.BaseStream.CopyToAsync(target, _processSettings.BufferSize, _cancellationTokenSource.Token);
                }
            }
            finally
            {
        ...
        public async Task ExecuteAsync(string argsString, PipeWriter target)
        {
            ...
            try
            {
                ...
                if (_processSettings.CaptureStderrInsteadOfStdout)
                {
                    await _process.StandardError.BaseStream.CopyToAsync(target, _cancellationTokenSource.Token);
                }
                else
                {
                    await _process.StandardOutput.BaseStream.CopyToAsync(target, _cancellationTokenSource.Token);
                }
            }
            finally
            {
        ...
        private ProcessStartInfo BuildOptions(string arguments)
        {
            ...
            processStartInfo.FileName = _processSettings.Filename;
            processStartInfo.Arguments = arguments;
            processStartInfo.UseShellExecute = false;
            processStartInfo.RedirectStandardOutput = true;
            processStartInfo.RedirectStandardError = true;
            processStartInfo.CreateNoWindow = true;
            processStartInfo.WorkingDirectory = _processSettings.WorkingDirectory ?? Environment.CurrentDirectory;
